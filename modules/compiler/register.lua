...
local randomStrings = require("modules.utils.randomStrings");
...
return function(Compiler)
    ...
    function Compiler:getVarRegister(scope, id, functionDepth, potentialId)
        ...
        if self.scopeFunctionDepths[scope] ~= functionDepth then
    ...
    function Compiler:encodePos(expr)
        return Ast.AddExpression(expr, Ast.NumberExpression(self.posKey));
    end
    ...
    function Compiler:setPos(scope, val)
        if not val then
            local v = Ast.IndexExpression(self:env(scope), randomStrings.randomStringNode(12));
            scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar);
            return Ast.AssignmentStatement({
                Ast.AssignmentVariable(self.containerFuncScope, self.posVar)
            }, {
        ...
        return Ast.AssignmentStatement({
            ...
            self:encodePos(Ast.NumberExpression(val))
        });
    end
