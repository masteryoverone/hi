...
return function(Compiler)
    function Compiler:compileTopNode(node)
        ...
        visitast(node, function(node, data)
            ...
            if varAccessLookup[node.kind] then
                if not node.scope.isGlobal then
                    if node.scope.__depth == nil then
                        node.scope.__depth = data.functionData.depth
                    end
        ...
        self:addStatement(self:setRegister(scope, self.varargReg, Ast.VariableExpression(self.containerFuncScope, self.argsVar)), {
            self.varargReg
        }, {}, false);
        local envReg = self:getVarRegister(self.scope, self.envVar, 0, nil);
        scope:addReferenceToHigherScope(self.scope, self.allocUpvalFunction);
        local allocExpr = Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.allocUpvalFunction), {});
        self:addStatement(self:setRegister(scope, envReg, allocExpr), {envReg}, {}, false);
        self:addStatement(self:setUpvalueMember(scope, self:register(scope, envReg), Ast.VariableExpression(self.scope, self.envVar)), {}, {envReg}, true);
        self:compileBlock(node.body, 0);
        if (self.activeBlock.advanceToNextBlock) then
            self:addStatement(self:setPos(self.activeBlock.scope, nil), {
                self.POS_REGISTER
            }, {}, false);
    ...
    function Compiler:compileFunction(node, funcDepth)
        ...
        for i, arg in ipairs(node.args) do
            if (arg.kind == AstKind.VariableExpression) then
                if (self:isUpvalue(arg.scope, arg.id)) then
                    ...
                    self:addStatement(self:setRegister(scope, argReg, Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.allocUpvalFunction), {})), {
                        argReg
                    }, {}, false);
                    self:addStatement(self:setUpvalueMember(scope, self:register(scope, argReg), Ast.IndexExpression(Ast.VariableExpression(self.containerFuncScope, self.argsVar), Ast.NumberExpression(i))), {}, {
                        argReg
                    }, true);
                else
                    local argReg = self:getVarRegister(arg.scope, arg.id, funcDepth, nil);
                    scope:addReferenceToHigherScope(self.containerFuncScope, self.argsVar);
                    self:addStatement(self:setRegister(scope, argReg, Ast.IndexExpression(Ast.VariableExpression(self.containerFuncScope, self.argsVar), Ast.NumberExpression(i))), {
                        argReg
                    }, {}, false);
                end
            else
        ...
        if isVarargFunction then
            ...
            retrieveExpression = Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.createVarargClosureVar), {
                Ast.NumberExpression(block.id + self.posKey),
                Ast.TableConstructorExpression(upvalueExpressions)
            });
        else
            local varScope, var = self:getCreateClosureVar(# node.args);
        ...
        self:addStatement(self:setRegister(scope, retReg, retrieveExpression), {
            retReg
        }, usedRegs, false);
        return retReg;
    end
    function Compiler:compileBlock(block, funcDepth)
        ...
        local scope = self.activeBlock.scope;
        for id, name in pairs(block.scope.variables) do
            ...
            if self:isUpvalue(block.scope, id) then
                ...
                self:addStatement(self:setRegister(scope, varReg, Ast.NilExpression()), {
                    varReg
                }, {}, false);
            end
