...
return function(self, statement, funcDepth)
    ...
    local forInLhs = {self:registerAssignment(scope, exprregs[3])};
    local forInWrites = {exprregs[3]};
    if varRegs[2] then
        forInLhs[2] = self:registerAssignment(scope, varRegs[2]);
        forInWrites[2] = varRegs[2];
    end
    self:addStatement(Ast.AssignmentStatement(forInLhs, {
        Ast.FunctionCallExpression(self:register(scope, exprregs[1]), {
            self:register(scope, exprregs[2]),
            self:register(scope, exprregs[3]),
        })
    }), forInWrites, {exprregs[1], exprregs[2], exprregs[3]}, true);
