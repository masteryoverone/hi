...
function Unparser:unparseBlock(block, tabbing)
	...
	for i, statement in ipairs(block.statements) do
		if (statement.kind ~= AstKind.NopStatement) then
			...
			if i ~= 1 then
				parts[# parts + 1] = ws;
				if (not self.prettyPrint and string.sub(statementCode, 1, 1) ~= ";") then
					statementCode = ";" .. statementCode;
				end
			end
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif (statement.kind == AstKind.DoStatement) then
		...
		push("do", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode, self:newline(false), self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)), "end");

	elseif (statement.kind == AstKind.WhileStatement) then
		local expressionCode = self:unparseExpression(statement.condition, tabbing);
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		push("while", self:whitespaceIfNeeded(expressionCode), expressionCode, self:whitespaceIfNeeded2(expressionCode), "do", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode, self:newline(false), self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)), "end");

	elseif (statement.kind == AstKind.RepeatStatement) then
		local expressionCode = self:unparseExpression(statement.condition, tabbing);
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		push("repeat", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode, self:whitespaceIfNeeded2(bodyCode, self:newline() .. self:tabs(tabbing, true)), "until", self:whitespaceIfNeeded(expressionCode), expressionCode);
	...
	elseif (statement.kind == AstKind.ForStatement) then
		...
		push(self:optionalWhitespace(), incrementByCode, self:whitespaceIfNeeded2(incrementByCode), "do", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode, self:newline(false), self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)), "end");
	...
	elseif (statement.kind == AstKind.ForInStatement) then
		...
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		push(self:whitespaceIfNeeded2(# parts > 0 and parts[# parts] or ""), "do", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode, self:newline(false), self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)), "end");

	elseif (statement.kind == AstKind.IfStatement) then
		local exprcode = self:unparseExpression(statement.condition, tabbing);
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		push("if", self:whitespaceIfNeeded(exprcode), exprcode, self:whitespaceIfNeeded2(exprcode), "then", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode);
		for i, eif in ipairs(statement.elseifs) do
			...
			push(self:newline(false), self:whitespaceIfNeeded2(lastPart, self:tabs(tabbing, true)), "elseif", self:whitespaceIfNeeded(exprcode), exprcode, self:whitespaceIfNeeded2(exprcode), "then", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode);
		end
		if (statement.elsebody) then
			bodyCode = self:unparseBlock(statement.elsebody, tabbing);
			local lastPart = # parts > 0 and parts[# parts] or "";
			push(self:newline(false), self:whitespaceIfNeeded2(lastPart, self:tabs(tabbing, true)), "else", self:whitespaceIfNeeded(bodyCode, self:newline(true)), bodyCode);
		end
	...
	elseif (statement.kind == AstKind.FunctionDeclaration) then
		...
		push(")");
		local bodyCode = self:unparseBlock(statement.body, tabbing);
		push(self:newline(false), bodyCode, self:newline(false), self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)), "end");
	...
	elseif self.luaVersion == LuaVersion.LuaU then
		...
		if operator then
			push(self:unparseExpression(statement.lhs, tabbing), self:optionalWhitespace(), operator, self:optionalWhitespace(), self:unparseExpression(statement.rhs, tabbing));
		else
...
function Unparser:unparseExpression(expression, tabbing)
	...
	local bitwiseOps = {
		...
		[AstKind.ShiftLeftExpression] = "<<",
		[AstKind.ShiftRightExpression] = ">>",
	}
	op = bitwiseOps[expression.kind]
	if op then
		k = expression.kind
		local lhs = self:unparseExpression(expression.lhs, tabbing);
		if (Ast.astKindExpressionToNumber(expression.lhs.kind) >= Ast.astKindExpressionToNumber(k)) then
	...
	if (expression.kind == k) then
		...
		for i, entry in ipairs(expression.entries) do
			p = true;
			local sep = self.prettyPrint and "," or ",";
